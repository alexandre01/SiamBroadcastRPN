...
from datasets import TrackingNet, ImageNetVID, PairSampler, CocoDetection, COCOPair
...
class Trainer(object):
    ...
    def __init__(self, net, cfg):
        ...
        transform = [
            ...
            Transform(context_amount=1.0, random_translate=True, random_resize=True, motion_blur=True)
        ]
        ...
        cocopair = COCOPair(coco, 8000)

        dataset = ConcatDataset([sampler, cocopair])
        ...
        validation_sampler = PairSampler([val_trackingnet, val_imagenet], transform=transform, pairs_per_video=1,
                                         frame_range=200)
        val_cocopair = COCOPair(coco, 200)
        val_dataset = ConcatDataset([validation_sampler, val_cocopair])

        if cfg.TRAIN.DEBUG_SEQ >= 0:  # When debugging on a single sequence, the validation is performed on the same one
            val_dataset = PairSampler([trackingnet], transform=transform, pairs_per_video=200)

        self.validation_dataloader = DataLoader(val_dataset, batch_size=cfg.TRAIN.BATCH_SIZE, num_workers=4,
    ...
    def train(self):
        ...
        for epoch in range(self.start_epoch, self.cfg.TRAIN.NUM_EPOCHS):
            ...
            for batch_idx, batch in enumerate(self.dataloader):
                ...
                z_mask, x_mask = mask_imgs(z, z_bbox), mask_imgs(x, xprev_bbox, use_mask=False)
                ...
                loss_loc, loss_cls, pos_accuracy, neg_accuracy, position_err, size_errors, pos_matches = self.criterion(pred, x_bbox)
    ...
    def compute_validation_metrics(self, iter_idx):
        ...
        for i, (z, x, z_bbox, x_bbox, xprev_bbox) in enumerate(self.validation_dataloader):
            ...
            # Display the first 25 images from the validation set.
            if i < 25:
