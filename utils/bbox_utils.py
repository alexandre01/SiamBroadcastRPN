...
class bbox_format:
    ...
    The available formats are:
        - x1y1wh: top-left coordinates, width and height.
        - cxcywh: center coordinates, width and height.
        - x1y1x2y2: top-left coordinates, bottom-right coordinates.
    """
    x1y1wh = "x1y1wh"
    cxcywh = "cxcywh"
    x1y1x2y2 = "x1y1x2y2"
...
def format_from_to(_bbox, source_format, target_format):
    ...
    """
    if source_format not in vars(bbox_format) or target_format not in vars(bbox_format):
        raise ValueError("Invalid source or target format")
...
def to_percentage_coords(bbox, img_shape):
    """
    bbox: (np array) in x1y1x2y2 format.
...
def to_absolute_coords(bbox, img_shape):
    """
    bbox: (Tensor) in x1y1x2y2 format.
...
def match(threshold, truths, priors, variances, labels, loc_t, conf_t, idx):
    ...
    Args:
        ...
        truths: (tensor) Ground truth boxes, Shape: [num_obj, 4+1].
...
def nms(boxes, scores, overlap=0.5, top_k=200):
    ...
    while idx.numel() > 0:
        ...
        # Add i to the kept indices, remove it from the search list and terminate the loop if no other index remains
        ...
        # load bboxes of next highest vals (whose indices correspond to the new idx list)
        with torch.no_grad():
        ...
        # store element-wise max with next highest score (snap the remaining bboxes to bbox i, in order to compute the intersection)
        ...
        # check sizes of xx1 and xx2.. after each iteration (compute the intersection area)
        w = torch.clamp(w, min=0.0)
        h = torch.clamp(h, min=0.0)
        inter = w * h

        # Compute the IoU = i / (area(a) + area(b) - i)
